...
namespace bx
{
	...
	// MSVC：含 C++ 对象展开的函数内不能使用 __try；将 SEH 隔离到仅 POD 的辅助函数（本地补丁）。
	static void raiseThreadNameExceptionMSVC(DWORD _threadId, LPCSTR _name)
	{
#	pragma pack(push, 8)
		struct ThreadName {
			DWORD type;
...
#	error "Not implemented!"
	...
	|| BX_PLATFORM_VISIONOS
		...
		{
#	if BX_COMPILER_MSVC
			raiseThreadNameExceptionMSVC(ti->m_threadId, m_name.getCPtr());
