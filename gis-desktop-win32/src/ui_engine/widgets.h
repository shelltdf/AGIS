...
/**
 * 全部具体 Widget 子类声明（单头文件）。
 *
 * - **通用控件**：与 Qt 基元对应的 `Window`、`Label`、`PushButton` 等，跨平台后端可复用。
 * - **AGIS 主窗口映射**：与 `main.cpp` / `map_engine` 中已实现的 HWND、菜单、Dock、地图叠层、对话框等一一对应的类型（原独立文件 `widgets_shell.h` 已合并入本文件；实现见 `widgets.cpp`）。
 */
...
// --- 枚举（布局 / Dock；供 DockArea、Splitter 等使用）---
