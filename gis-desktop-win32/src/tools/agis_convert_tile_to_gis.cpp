...
void PrintHelp() {
  ...
  if (zh) {
    std::wcout << L"用法:\n"
               ...
               << L"  --output <path>              输出 GIS 路径（如 GeoTIFF）\n\n"
               << L"通用选项:\n"
               << L"  --help, -h, /?               显示帮助\n";
    PrintConvertCliHelpGrouped(
        std::wcout, true,
        L"  瓦片→GIS；主类型一般为 tile。\n",
    ...
    std::wcout << L"Usage:\n"
               ...
               << L"  --output <path>              Output GIS path (e.g. GeoTIFF)\n\n"
               << L"General:\n"
               << L"  --help, -h, /?               Show help\n";
    PrintConvertCliHelpGrouped(
        std::wcout, false,
        L"  Tiles → GIS; major type is usually tile.\n",
        L"  xyz | tms | wmts | mbtiles | gpkg | 3dtiles (default: xyz).\n"
        L"  Directory subtypes: pick a folder; mbtiles/gpkg: pick a file.\n",
...
int RunDirect(const wchar_t* title, const ConvertArgs& args) {
  ...
  const int rc = ConvertTileToGis(args);
...
int wmain(int argc, wchar_t** argv) {
  ...
  return RunDirect(L"TILE -> GIS", args);
}
