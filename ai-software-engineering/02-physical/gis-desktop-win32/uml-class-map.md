...
**源码根**：[`map_engine/`](../../../map_engine/)（实现 `map_engine/src/*.cpp`，公开头 `map_engine/include/map_engine/*.h`）。

**说明**：`MapLayer` 的 GDAL 具体实现类仅在 [`map_engine.cpp`](../../../map_engine/src/map_engine.cpp) 的 `agis_detail` 命名空间中定义，未暴露在公开头文件中；物理层类图仍列出二者以便与 `std::unique_ptr<MapLayer>` 所有权关系对齐。
