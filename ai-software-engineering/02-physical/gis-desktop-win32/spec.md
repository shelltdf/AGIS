...
- **菜单路径**：`工具 → 数据转换`。  
- **窗口结构**：上半区三列（输入 / 中间参数 / 输出），下半区命令行、多段进度与日志。中间列为 **四段分组框**，与四条 CLI 轴一一对应：① **输入类型**（`--input-type`，含 `vector-mode` / `elev-horiz-ratio` / 输入侧 `coord-system` 等仅在适用时出现）；② **输入子类型**（`--input-subtype`，含 `raster-max-dim` 等）；③ **输出类型**（`--output-type`，含 `target-crs`、输出侧 `coord-system`、`output-unit`、`mesh-spacing`、`texture-format`、`obj-fp-type` 等）；④ **输出子类型**（`--output-subtype`，含 `tile-levels` 等）。各段内控件随当前四元选择 **动态显隐与重排**（`LayoutConvertMidColumn`）；空段的分组框隐藏。**命令预览**（`BuildConvertCommandLine`）：文本框**第一行**（无前置说明）即为 `AssembleConvertProcessCommandLine` 与「开始转换」完全相同的**单行可执行命令**；其下为简短说明，再为「路径 + 【1】–【4】」分项（与 `--help` 分块及中间列 ①–④ 一致）。**「复制命令」** 仅复制该首行（与进程实际参数一致；`raster-max-dim` 为 0、`tile-levels` 为 `auto` 等与后端默认值等价时省略）。未传选项由 `convert_backend_common` 的 `ParseConvertArgs` 默认填充（如 `coord-system`→`projected`、`vector-mode`→`geometry` 等）。**布局**：「复制命令」与「开始转换」同一行靠右，其下为「复制输出」。  
...
- **瓦片预览（内置）**：索引扫描上限约 **12000**。**已实现**：**XYZ 目录**四叉拼图与交互；**TMS** 目录在检测到 **`tms.xml`** 或 **`README.txt`** 含 **`protocol=tms`**（与 `agis_convert_gis_to_tile` 写出一致）时，将磁盘行号文件映射为 **XYZ 显示**（北向上）。**缩放**：滚轮 **切换磁盘上的 Z 级**（类在线栅格图：一级一屏瓦片），**光标下归一化位置对齐**；**Shift+滚轮** 以视口中心换级；**Ctrl+滚轮** 仅调 **每瓦片像素边长**（默认约 **256px**/瓦片，用于微调清晰度）。**内存**：已解码瓦片 **LRU**（约 **160** 块为上限），**换 Z 时丢弃非当前层的位图缓存**，避免多层堆积。**单文件 `*.mbtiles` / `*.gpkg`**：在 **`AGIS_USE_GDAL=ON`** 时由 **GDAL 栅格打开**下采样缩略预览（全球墨卡托拼图，非 z/x/y 交互）；纯矢量 GPKG 或缺驱动时失败并提示。**绘制**：`WM_ERASEBKGND` 跳过默认擦除、`WM_PAINT` 内存位图 **BitBlt** 以减少 XYZ 拼图闪烁。**非 3dtiles** 下 **3D Tiles** 仍为 **元数据模式**。**另**：子类型 **`3dtiles`** 的 **三维网格/点云近似** 在 **模型预览窗**。**未实现**：容器内按 zoom 交互拼瓦片（MBTiles/GPKG 仍单张缩略）、全规格 3D Tiles 语义（如全部量化/批次属性变体）。
