...
- **菜单路径**：`工具 → 数据转换`。  
- **窗口结构**：上半区三列（输入 / 中间参数 / 输出），下半区命令行、多段进度与日志。中间参数列拆为两组：**输入转换参数** 与 **输出转换参数**；其中输出组聚合当前子类型可用参数（坐标系、矢量策略、高程/水平比、目标投影、输出单位、Mesh 间距、贴图格式、`raster-max-dim`）并实时映射到 CLI 预览。  
...
- **执行流程（非阻塞）**：点击执行后以后台进程启动后端转换，UI 线程不阻塞；通过定时轮询进程状态更新进度与最终退出码。
...
- **预览背景与刷新**：3D 视口默认背景为浅灰色；窗口定时触发重绘并立即刷新，保证 FPS/性能信息持续更新。  
